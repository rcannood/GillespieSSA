...
#' )
#'
#' @details Although \code{ssa} can be invoked by only specifying the system
...
#' * \code{"D"} --- Direct method (default method),
#' * \code{"ETL"} - Explicit tau-leap,
#' * \code{"BTL"} --- Binomial tau-leap, or
#' * \code{"OTL"} --- Optimized tau-leap.
#' @param simName optional text string providing an arbitrary name/label for
#' the simulation.
#' @param tau step size for the \code{ETL} method (\eqn{>0}).
#' @param f coarse-graining factor for the \code{BTL} method (\eqn{>1}) where a
#'   higher value results in larger step-size.
#' @param epsilon accuracy control parameter for the \code{OTL} method (\eqn{>0}).
#' @param nc critical firing threshold for the \code{OTL} method (positive integer).
...
#' @param dtf \code{D} method threshold factor for the \code{OTL} method. The
...
#' @param nd number of single-reaction steps performed using the Direct method during \code{otl} suspension (See step 3, page 3, Cao et al. 2006).
...
#' @param consoleInterval (approximate) interval at which \code{ssa} produces
...
#'    - \code{finalTime} = if the simulation reached the maximum simulation time \code{tf},
#'    - \code{extinction} = if the population size of all states is zero,
#'    - \code{negativeState} = if one or several states have a negative population size (can occur in the ETL method),
#'    - \code{zeroProp} = if all the states have a zero propensity function,
#'    - \code{maxWallTime} = if the maximum wall time has been reached. Note the termination status may have more than one message.
...
#'  * `stats$SuspendedTauLeaps`: number of steps performed using the Direct method due to \code{OTL} suspension (only applicable for the \code{OTL} method).
#'  * `arg$...`: sub-list with elements containing all the arguments and their values used to invoke \code{ssa} (see Usage and Arguments list above).
#'
...
#' }
#'
#' @seealso \link{GillespieSSA-package}, \code{\link{ssa.direct}}, \code{\link{ssa.etl}}, \code{\link{ssa.btl}}, \code{\link{ssa.otl}}
#'
